...
for i in range(0,data.shape[1]):
    if i == 0:
        ...
        data.iloc[:,i] = y2
data2 = data[list(data.columns)[1:]]
data2.to_csv('temp3.csv',index=False,encoding = 'cp949')
data2 = data2.drop(['T1goldEarned','T2goldEarned','T1champLevel','T2champLevel'],axis =1)
from sklearn.model_selection import train_test_split

X_train, X_test, y_train, y_test = train_test_split(data2, np.array(data['sT1Win'].tolist()), test_size=0.25, stratify=np.array(data['sT1Win'].tolist()), random_state=123456)
...
plt.xticks(range(X_train.shape[1]), X_train.columns[indices], rotation=25)
